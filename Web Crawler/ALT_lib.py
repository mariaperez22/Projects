...
############################################################################################################## 
## Indexacion y comprobacion correcta integracion del Speller con el proyecto de SAR                        ##
## python3 ALT_Indexer.py ./files/100 index_100.bin                                                      ##
## python3 ALT_Searcher.py index_100.bin -T ./results/result_100_levenshtein_1.txt -s -t 1 -d "levenshtein" ##
##############################################################################################################
...
class SAR_Indexer:
    ...
    all_atribs = ['urls', 'index', 'sindex', 'ptindex', 'docs', 'weight', 'articles',
                  'tokenizer', 'stemmer', 'show_all', 'show_snippet', 'use_stemming','use_spelling', 'speller'] 
    ...
    def set_spelling(self, use_spelling:bool, distance:str=None, threshold:int=None):
        ...
        """
        vocabulary = sorted(self.index['all'].keys())
        self.use_spelling = use_spelling

        if not use_spelling:
            # Para asegurarnos de que no se usa el Speller en caso de use_spelling siendo False
            self.speller = None 

        self.speller = SpellSuggester(opcionesSpell, vocab=vocabulary,
                                      default_distance=distance, default_threshold=threshold)
        # Necesario actualizar los atributos de la clase porque se acceden a ellos en get_posting
    ...
    def get_posting(self, term:str, field:str='all'):
        ...
        if term not in self.index[field] and self.use_spelling:
            sugerencias = self.speller.suggest(term,self.distance,self.threshold,True)
